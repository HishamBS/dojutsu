...
if command -v agent-mux >/dev/null 2>&1; then
  ...
  MUX_ARGS=(--stream --engine "$ENGINE" --effort high --cwd "$(pwd)" --max-turns "$COMPUTED_TURNS" --timeout "$COMPUTED_TIMEOUT_SEC")
  if [[ -n "$MODEL" ]]; then
  ...
  # Parse JSON output contract (Go agent-mux v2: status field replaces success/completed)
  MUX_STATUS=$(echo "$MUX_RESULT" | jq -r '.status // "failed"' 2>/dev/null)

  if [[ "$MUX_STATUS" != "completed" ]]; then
    MUX_ERROR=$(echo "$MUX_RESULT" | jq -r '.error.message // "unknown error"' 2>/dev/null)
